...
function POS() {
  ...
  return (
    <div className="p-6 grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6 h-full">
      <div>
        <PageHeader title="Checkout · INV-2091" subtitle="Hasini Wijesuriya · Toyota Aqua · CAR-4521" />
              ...
              {lines.map((l, idx) => (
                <tr key={l.id}>
                  <td className="px-4 py-3">
                    ...
                    <div className="text-[11px] text-muted-foreground">{l.category} · {l.durationMin}m</div>
                  </td>
                  ...
                  <td className="px-3 py-3 text-right font-mono text-primary">-{l.discount.toLocaleString()}</td>
                  <td className="px-3 py-3 text-right font-mono font-semibold">{(l.price * l.qty - l.discount).toLocaleString()}</td>
        ...
        <div className="mt-6">
          <h3 className="font-display text-sm font-bold mb-3 uppercase tracking-wider text-muted-foreground">Recent Invoices</h3>
              ...
              <tbody className="divide-y divide-border">
                {INVOICES.map((i) => (
                  <tr key={i.id} className="hover:bg-muted/40">
                    ...
                    <td className="px-3 py-2.5 text-right font-mono font-semibold">LKR {i.total.toLocaleString()}</td>
                    <td className="px-3 py-2.5 text-muted-foreground">{i.method}</td>
                    <td className="px-3 py-2.5"><StatusChip variant={statusVariant(i.status)}>{i.status}</StatusChip></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <aside className="rounded-xl border border-border bg-card shadow-card p-5 h-fit sticky top-4">
        <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Customer</div>
        ...
        <div className="flex items-baseline justify-between py-4">
          ...
          <span className="font-display text-2xl font-extrabold text-primary">LKR {total.toLocaleString()}</span>
        </div>
        ...
        <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-2">Payment Method</div>
          ...
          {([
            ...
            <button
              ...
              className={cn(
                ...
                method === v ? "border-primary bg-primary/10 text-primary" : "border-input hover:bg-accent",
              )}
            >
