        ...
        {STAFF.map((s) => (
          <div key={s.id} className="rounded-xl border border-border bg-card p-4 shadow-card hover:shadow-elevated transition-shadow">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              <Stat label="Jobs Today" value={Math.floor(Math.random() * 8)} />
              <Stat label="Revenue" value={`LKR ${(Math.floor(Math.random() * 80) + 20)}k`} />
              <Stat label="On-time" value={`${85 + Math.floor(Math.random() * 15)}%`} />
            </div>
            <div className="mt-3 flex justify-between text-[11px] text-muted-foreground border-t border-border pt-3">
              <span>PIN · <span className="font-mono">●●●●{s.role === "Admin" ? "●●" : ""}</span></span>
