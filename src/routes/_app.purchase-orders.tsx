...
function CreatePOForm({ onSave, onCancel }: { onSave: (data: Omit<PurchaseOrder, "id" | "poNumber" | "createdAt">) => void; onCancel: () => void }) {
    ...
    <form onSubmit={submit} className="rounded-lg border border-border bg-muted/30 p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <div>
          ...
          <input className={inp} value={supplier} onChange={(e) => setSupplier(e.target.value)} placeholder="Supplier name" required />
        </div>
        <div>
          <label className={lbl}>Raised By</label>
          <input className={inp} value={createdBy} onChange={(e) => setCreatedBy(e.target.value)} placeholder="Your name" />
        </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <select className={cn(inp, "flex-1")} value={itemPick} onChange={(e) => { setItemPick(e.target.value); addLine(e.target.value); }}>
            <option value="">— Select item to add —</option>
            {inventory.filter((i) => !lines.find((l) => l.inventoryItemId === i.id)).map((i) => (
              <option key={i.id} value={i.id}>{i.name} ({i.sku}) — Stock: {i.stock} {i.unit}</option>
            ))}
          </select>
        </div>
      </div>
        ...
        <div className="overflow-hidden rounded-lg border border-border">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Item / SKU</th>
                <th className="px-3 py-2 text-center text-xs font-medium text-muted-foreground">Unit</th>
                <th className="px-3 py-2 text-center text-xs font-medium text-muted-foreground">Qty</th>
                <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Unit Cost (LKR)</th>
                <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Line Total</th>
            ...
            <tbody>
              {lines.map((l, idx) => (
                <tr key={l.inventoryItemId} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    <input type="number" min={1} className="w-20 rounded border border-input bg-background px-2 py-1 text-center text-sm" value={l.qtyOrdered} onChange={(e) => updateLine(idx, "qtyOrdered", parseInt(e.target.value) || 1)} />
                  </td>
                  <td className="px-3 py-2">
                    <input type="number" min={0} className="w-28 rounded border border-input bg-background px-2 py-1 text-right text-sm" value={l.unitCost} onChange={(e) => updateLine(idx, "unitCost", parseFloat(e.target.value) || 0)} />
                  </td>
                  <td className="px-3 py-2 text-right font-medium">{fmtLKR(l.unitCost * l.qtyOrdered)}</td>
                  <td className="px-3 py-2">
                    <button type="button" onClick={() => removeLine(idx)} className="rounded p-1 text-muted-foreground hover:text-destructive"><X className="h-4 w-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t-2 border-border bg-muted/30">
                <td colSpan={4} className="px-3 py-2 text-right text-sm font-semibold">Order Total</td>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button type="submit" disabled={lines.length === 0} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
          Create PO
        </button>
      </div>
    </form>
...
function ReceivePanel({
  ...
  function setAll() {
    setReceived(Object.fromEntries(po.lines.map((l) => [l.inventoryItemId, l.qtyOrdered - l.qtyReceived])));
  }
  ...
  return (
    <div className="mt-3 rounded-lg border border-border bg-background p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold flex items-center gap-1.5"><PackageCheck className="h-4 w-4" /> Receive Stock</p>
        <button type="button" onClick={setAll} className="text-xs text-primary hover:underline">Set all to ordered qty</button>
      </div>
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-muted/50">
            <th className="px-3 py-1.5 text-left text-xs font-medium text-muted-foreground">Item</th>
            <th className="px-3 py-1.5 text-center text-xs font-medium text-muted-foreground">Ordered</th>
            <th className="px-3 py-1.5 text-center text-xs font-medium text-muted-foreground">Already Received</th>
            <th className="px-3 py-1.5 text-center text-xs font-medium text-muted-foreground">Receiving Now</th>
          </tr>
        </thead>
        <tbody>
          {po.lines.map((l) => {
            ...
            return (
              <tr key={l.inventoryItemId} className="border-t border-border">
                ...
                <td className="px-3 py-2 text-center">{l.qtyOrdered} {l.unit}</td>
                ...
                <td className="px-3 py-2 text-center">
                  <input
                    ...
                    onChange={(e) => setReceived((r) => ({ ...r, [l.inventoryItemId]: Math.min(remaining, parseInt(e.target.value) || 0) }))}
                  />
                </td>
              </tr>
            );
      ...
      <div className="flex justify-end gap-2">
        ...
        <button type="button" onClick={confirm} className="flex items-center gap-1.5 rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700">
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-4 py-3 text-sm text-muted-foreground">{po.lines.length} item{po.lines.length !== 1 ? "s" : ""}</td>
        <td className="px-4 py-3 text-sm font-semibold">{fmtLKR(total)}</td>
        <td className="px-4 py-3">
          <span className={cn("rounded-full px-2.5 py-0.5 text-xs font-medium", STATUS_BADGE[po.status])}>{po.status}</span>
        </td>
        <td className="px-4 py-3">
          <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => downloadPOPDF(po)} className="rounded p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" title="Download PDF">
              <FileDown className="h-4 w-4" />
            </button>
            {po.status === "Draft" && (
              <button onClick={() => { if (confirm("Delete this draft PO?")) deletePurchaseOrder(po.id); }} className="rounded p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" title="Delete">
        ...
        <tr className="border-t border-border">
          <td colSpan={7} className="bg-muted/10 px-6 py-4">
            ...
            <div className="mb-3 grid grid-cols-2 gap-x-8 text-sm sm:grid-cols-4">
              {po.createdBy && <div><span className="text-muted-foreground">Raised by: </span>{po.createdBy}</div>}
              {po.sentAt && <div><span className="text-muted-foreground">Sent: </span>{fmtDate(po.sentAt)}</div>}
              {po.receivedAt && <div><span className="text-muted-foreground">Received: </span>{fmtDate(po.receivedAt)}</div>}
              {po.notes && <div className="col-span-2 sm:col-span-4 text-muted-foreground">{po.notes}</div>}
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Item / SKU</th>
                    <th className="px-3 py-2 text-center text-xs font-medium text-muted-foreground">Unit</th>
                    <th className="px-3 py-2 text-center text-xs font-medium text-muted-foreground">Ordered</th>
                ...
                <tbody>
                  {po.lines.map((l) => (
                    <tr key={l.inventoryItemId} className="border-t border-border">
                      ...
                      <td className="px-3 py-2 text-center">
                        <span className={cn("font-medium", l.qtyReceived >= l.qtyOrdered ? "text-green-600" : l.qtyReceived > 0 ? "text-amber-600" : "text-muted-foreground")}>
                          {l.qtyReceived}
                        </span>
                      </td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-border bg-muted/30">
                    <td colSpan={5} className="px-3 py-2 text-right text-sm font-semibold">Order Total</td>
                    <td className="px-3 py-2 text-right text-sm font-bold">{fmtLKR(total)}</td>
                  </tr>
                </tfoot>
              </table>
            ...
            {receiving && (
              <ReceivePanel
                po={po}
                onConfirm={(lines) => { receivePO(po.id, lines); setReceiving(false); }}
            ...
            {!receiving && (
              <div className="flex flex-wrap gap-2">
                <button onClick={() => downloadPOPDF(po)} className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-sm hover:bg-muted">
                  <FileDown className="h-4 w-4" /> Download PDF
                </button>
                {canSend && (
                  <button onClick={markSent} className="flex items-center gap-1.5 rounded-md bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">
                    <Send className="h-4 w-4" /> Mark as Sent
                  </button>
                )}
                {canReceive && (
                  <button onClick={() => setReceiving(true)} className="flex items-center gap-1.5 rounded-md bg-green-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-green-700">
                    <PackageCheck className="h-4 w-4" /> Receive Stock
                  </button>
                )}
                {canCancel && (
                  <button onClick={cancel} className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10">
...
function PurchaseOrdersPage() {
  ...
  function createAutoPOs() {
    ...
    if (items.length === 0) { alert("No low-stock items with a supplier set."); return; }
    ...
    if (!confirm(`Create ${supplierCount} PO${supplierCount > 1 ? "s" : ""} for ${items.length} low-stock item${items.length > 1 ? "s" : ""}?`)) return;
  ...
  const filtered = purchaseOrdersList.filter((po) => filterStatus === "All" || po.status === filterStatus);

  const draft = purchaseOrdersList.filter((p) => p.status === "Draft").length;
  const sent = purchaseOrdersList.filter((p) => p.status === "Sent" || p.status === "Partially Received").length;
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">Manage supplier purchase orders and receive stock directly to inventory</p>
        </div>
        <div className="flex gap-2">
      ...
      {showForm && (
        <CreatePOForm
          onSave={(data) => { addPurchaseOrder(data); setShowForm(false); }}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: "Sent / In Transit", value: sent, icon: AlertTriangle, cls: sent > 0 ? "text-amber-500" : "" },
      ...
      {totalCommitted > 0 && (
        <div className="flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800/40 dark:bg-blue-900/20">
          ...
          <p className="text-sm text-blue-700 dark:text-blue-300">
            <span className="font-semibold">{fmtLKR(totalCommitted)}</span> committed on open purchase orders
          </p>
        </div>
      )}
      ...
      {/* Filter */}
      <div className="flex flex-wrap gap-2">
        {(["All", "Draft", "Sent", "Partially Received", "Received", "Cancelled"] as const).map((s) => (
          <button
            key={s}
            onClick={() => setFilterStatus(s)}
            className={cn("rounded-md px-3 py-1.5 text-sm font-medium transition-colors", filterStatus === s ? "bg-primary text-primary-foreground" : "border border-border hover:bg-muted")}
          >
            {s}
          </button>
        ))}
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <table className="w-full">
          <thead className="bg-muted/50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">PO #</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-16 text-center text-muted-foreground">
                  ...
                  {lowStockItems.length > 0 && (
                    <p className="mt-1 text-xs">Use <strong>Auto PO</strong> to generate orders from low-stock items</p>
                  )}
                </td>
              </tr>
            ) : (
